...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('../lib/hyperexif');
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('../lib/hyperfunc');
...
try {
  ...
  const client = hyperConnect({
    logger: pino({ level: "silent" }),
    printQRInTerminal: false,
    browser: ["HYPER-AI", "Safari", "5.1.7"],
    auth: state,
  ...
  if (autobio === 'on') {
    setInterval(() => {
      ...
      client.updateProfileStatus(
        `📅 𝙳𝙰𝚃𝙴/𝚃𝙸𝙼𝙴 ⌚️  ${date.toLocaleString('en-US', { timeZone: 'Africa/Nairobi' })}  ⏰️ 𝙳𝙰𝚈 ⏰️  ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/Nairobi'})}. 𝐇𝐘𝐏𝐄𝐑 𝐁𝐎𝐓 𝐈𝐒 𝐂𝐔𝐑𝐄𝐍𝐓𝐋𝐘 𝐀𝐂𝐓𝐈𝐕𝐄 𝐀𝐍𝐃 𝐑𝐔𝐍𝐍𝐈𝐍𝐆⚡.`
      );
    }, 10 * 1000);
  }
 ...
 if (autoview === 'on' && autolike === 'on' && mek.key && mek.key.remoteJid === "status@broadcast") {
        const jay = await client.decodeJid(client.user.id);
...
if (!client.public && !mek.key.fromMe && chatUpdate.type === "notify") return;
      ...
      const hyper = require("../action/hyper");
      hyper(client, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });
...
try {
  ...
  console.error("❌ Failed to initialize database:", err.message || err);
      ...
      console.log(color("Congrats, HYPER-BOT has successfully connected to this server", "green"));
      console.log(color("Follow me on Instagram as Sir jay0001", "red"));
      console.log(color("Text the bot number with menu to check my command list"));
      const Texxt = `✅ 𝗖𝗼𝗻𝗻𝗲𝗰𝘁𝗲𝗱  ╍>〚𝗛𝗬𝗣𝗘𝗥-𝗕𝗢𝗧〛\n`+`👥 𝗠𝗼𝗱𝗲  ╍>〚${mode}〛\n`+`👤 𝗣𝗿𝗲𝗳𝗶𝘅  ╍>〚 ${prefix} 〛`
